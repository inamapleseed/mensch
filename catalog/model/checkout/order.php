...
class ModelCheckoutOrder extends Model {
	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				if( !isset($product['redeem']) ){
					$product['redeem'] = "0";
				}
				if( !isset($product['redeem_point']) ){
					$product['redeem_point'] = 0;
				}


				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', sku='". $this->db->escape($sku) ."', redeem = '".(int)$product['redeem']."', redeem_point = '".(int)$product['redeem_point']."'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			// If current order status is not processing or not complete but new status is processing or complete then commence completing the order
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				// Stock subtraction
				$order_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "' AND subtract = '0'");

				foreach ($order_product_query->rows as $order_product) {
					$this->db->query("UPDATE " . DB_PREFIX . "order_product SET subtract = '1' WHERE order_product_id = '" . (int)$order_product['order_product_id'] . "'");
			...
			//auto send voucher email when order is completed status
			if(in_array($order_status_id,$this->config->get('config_complete_status'))){
				$voucher_info = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_voucher WHERE order_id = '" . (int)$order_id . "'");
				if($voucher_info->num_rows){
					foreach($voucher_info->rows as $voucher){
						$this->sendVoucher($voucher['voucher_id']);
					}
				}
			}
			...
			if ( in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, $this->config->get('config_cancel_status')) ) {
				// Restock
				$product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "' AND subtrack = '1'");

				foreach($product_query->rows as $product) {
					$this->db->query("UPDATE `" . DB_PREFIX . "order_product` SET subtrack = '0' WHERE order_product_id = '" . (int)$product['order_product_id'] . "'");
			...
			if (!$order_info['order_status_id'] && $order_status_id && !in_array($order_status_id, $this->config->get('config_cancel_status'))) {
	...
	public function sendVoucher($voucher_id) {
		...
		if ($voucher_info) {
			...
			if ($order_info) {
				...
				$voucher_theme_info = $this->model_extension_total_voucher_theme->getVoucherTheme($voucher_info['voucher_theme_id']);
	...
	public function getOrderDeliveryDateSlot($delivery_date, $delivery_time = ""){
		$sql = "SELECT count(*) AS total FROM ".DB_PREFIX."order WHERE (order_status_id IN (".implode(",",$this->config->get('config_processing_status')).") OR order_status_id IN (".implode(",",$this->config->get('config_complete_status')).")) AND delivery_date = '".$delivery_date."'";
		
		if($delivery_time){
			$sql .= " AND delivery_time = '".$delivery_time."'";
		}
		
		$order_info = $this->db->query($sql);
		return $order_info->row['total'];
	}
}