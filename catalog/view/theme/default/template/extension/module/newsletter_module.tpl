<div class="newsletter-module<?= $uniqid ?> newsletter-module">
    <form id="form-newsletter<?= $uniqid ?>">
        <div class="pd-b15 text-center title"><div class="f60 orange pd-b30"><?= $title ?></div></div>
        <?php if($description) { ?>
		...
		<?php }else{ ?>
		<div class="flex mg-b10 input-wrap">
			<div class="flex-1 email-field"><input type="email" id="newsletter-input-email<?= $uniqid ?>" name="email" value="<?= $email ?>" class="form-control w100" placeholder="<?= $email_field_placeholder_text; ?>" /></div>
			<div class="text-center text-sm-right"><button type="button" id="submit-newsletter<?= $uniqid ?>" data-loading-text="<?= $text_loading; ?>" class="transition btn btn-primary"><?= $submit_button_field_text; ?></button></div>
		</div>
		<?php } ?>
	</form>
</div>
