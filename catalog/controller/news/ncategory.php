...
class ControllerNewsNcategory extends Controller {  
	public function index() { 
		...
		$default_title = $this->config->get('config_article_main_title');	
		...
		if($data['article_listing_layout'] == 'layout_3') {
	...
	protected function getPageContent($settings) {	
		...
		$data['config_article_title'] = $this->config->get('config_article_title');		
		...
		$settings['news_limit'] = $this->config->get('config_featured_article_limit');

		$data['display_style'] = $this->config->get('ncategory_bnews_display_style');
		$data['news_archive'] = $this->load->controller('extension/module/news_archive');
		// debug($data['news_archive']);

		$data['featured_news'] = $this->load->controller('extension/module/news_latest', $settings);
		...
		if (version_compare(VERSION, '2.2.0.0') >= 0) {
			return $this->load->view('news/ncategory_'.$data['article_listing_layout'], $data);
		} else {

			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/news/ncategory.tpl_'.$data['article_listing_layout'])) {
				return $this->load->view($this->config->get('config_template') . '/template/news/ncategory.tpl_'.$data['article_listing_layout'], $data);
			} else {
				return $this->load->view('default/template/news/ncategory.tpl_'.$data['article_listing_layout'], $data);
			}
		}
	  }
