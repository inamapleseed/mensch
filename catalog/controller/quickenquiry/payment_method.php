<?php  
class ControllerQuickEnquiryPaymentMethod extends Controller {
  	...
  	public function index() {
		...
		if (!empty($payment_address)) {
			...
			$recurring = $this->enquiry->hasRecurringProducts();
		...
		$this->response->setOutput($this->load->view('quickenquiry/payment_method', $data));
  	}
	...
	public function validate() { 
		...
		if (empty($payment_address)) {
			$json['redirect'] = $this->url->link('quickenquiry/checkout', '', true);
		}
	...
	public function ovewrite(){
		$json = array();

		$this->load->language('quickcheckout/overwrite');

		$this->session->data['payment_method'] = array(
			'code'       => 'enquiry_checkout',
			'title'      => $this->language->get('text_enquiry_checkout'),
			'terms'      => '',
			'sort_order' => 0
		);
