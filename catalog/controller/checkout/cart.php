		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'edit_cart' => $this->url->link('product/product', 'product_id=' . $product['product_id']."&cart_id=".$product['cart_id']),
				);
			}
		...
		if ($product_info) {
			if (isset($this->request->post['quantity']) && ((int)$this->request->post['quantity'] >= $product_info['minimum'])) {
				...
				if(isset($this->request->post['option'])){
					$existing_cart_quantity = $this->cart->checkOptionStock($product_id,'',$this->request->post['option'],'');
				}else{
					$existing_cart_quantity = $this->cart->checkOptionStock($product_id);
				}
			    $total_quantity = $quantity  + $existing_cart_quantity;
			} else {
				$quantity = (int)$this->request->post['quantity'];
				$json = array(
			...
			// Custom
			if($total_quantity > $product_info['quantity']){
				$json = array(
				...
				if($product_option_found !== false){
					...
					if($product_option_value_found !== false){
						if(isset($product_options[$product_option_found]['product_option_value'][$product_option_value_found]['subtract']) && $product_options[$product_option_found]['product_option_value'][$product_option_value_found]['subtract'] == 1){
							if($total_quantity > $product_options[$product_option_found]['product_option_value'][$product_option_value_found]['quantity']){
								$json = array(
									'error_outofstock_title'	=>	$this->language->get('error_outofstock_title'),
									'error_outofstock'			=>	$this->language->get('error_outofstock'),
								);
			...
			if(isset($this->request->post['option'])){
				...
				if($ro_for_product){
					foreach($ro_for_product as $r_product){
						if($total_quantity > $r_product['quantity']){
							$json = array(
								'error_outofstock_title'	=>	$this->language->get('error_outofstock_title'),
								'error_outofstock'			=>	$this->language->get('error_outofstock'),
							);
			...
			if (!$json) {
				...
				if(isset($this->request->post['existing_cart']) && $this->request->post['existing_cart'] > 0){
					$this->cart->updateCart($this->request->post['product_id'], $quantity, $option, $recurring_id,$this->request->post['existing_cart']);
				}else{
					$this->cart->add($this->request->post['product_id'], $quantity, $option, $recurring_id);
				}
				if(isset($this->request->post['existing_cart']) && $this->request->post['existing_cart'] > 0){
					$json['success_title'] = $this->language->get('update_title');
					$json['success'] = sprintf($this->language->get('text_update'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], $this->url->link('checkout/cart'));
				}else{
