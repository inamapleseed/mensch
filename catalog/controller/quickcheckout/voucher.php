...
class ControllerQuickCheckoutVoucher extends Controller {
	public function index() {
		...
		$cart_infos = $this->cart->getProducts();
		foreach($cart_infos as $cart_info){
			if($cart_info['redeem'] == 1){
				$points = $points - ($cart_info['redeem_point'] * $cart_info['quantity']);
			}
		}
		$data['entry_reward'] = sprintf($this->language->get('entry_reward'), $max, $points);
		
		$this->load->model('extension/total/coupon');
		$data['coupon_lists'] = $this->model_extension_total_coupon->getCouponList();
	...
	public function validateCoupon() {
		...
		if ($this->request->post['coupon'] == '') {
			...
			$coupon_info = $this->model_extension_total_coupon->getCoupon($this->request->post['coupon'], 0, true);
			
			//if (!$coupon_info) {			
				//$json['error']['warning'] = $this->language->get('error_coupon');
			//}
			if($coupon_info && !$coupon_info['valid']) {
