...
class ControllerExtensionPaymentOmise extends Controller {
	...
	public function checkout() {
		if (isset($this->request->post['omise_token'])) {
			...
			// convert total to correct currency
			$total = $this->currency->convert($order_info['total'],$order_info['currency_code'],$this->config->get('config_currency'));

			$order_total = $this->currency->format($total, $order_info['currency_code'], '', false);
						...
						if(isset($this->request->post['selected_omise_token'])){
						       $omise_charge = OmiseCharge::create(
        							array(
										...
										"amount"      => OmisePluginHelperCharge::amount($this->config->get('config_currency'), $order_total), // to use SGD
            				...
            				if ($omise_keys) {
    						    ...
    						    $omise_charge = OmiseCharge::create(
        							array(
        								"amount"      => OmisePluginHelperCharge::amount($this->config->get('config_currency'), $order_total),
        								"currency"    =>$this->config->get('config_currency'),
        								"description" => $this->request->post['description'],
        								"return_uri"  => $this->url->link('extension/payment/omise/checkoutcallback&order_id='.$order_id, '', 'SSL'),
		...
		try {
			...
			if ($order['payment_code'] == 'omise') {
		        ...
		        if ($order['order_status_id'] != 1) {
					throw new Exception('Order status MUST be `Order Received` to be updated.');
				}
				// if ($order['order_status_id'] != 0) {
				// 	throw new Exception('Order status MUST be `Processing` to be updated.');
				// }
