        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-coupon" class="form-horizontal">
          ...
          <div class="tab-content">
            <div class="tab-pane active" id="tab-general">
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label" for="input-max_total"><span data-toggle="tooltip" title="<?php echo "Maximan Cap Redeem Amount"; ?>"><?php echo "Maximan Redeem Amount"; ?></span></label>
                <div class="col-sm-10">
                  <input type="text" name="max_total" value="<?php echo $max_total; ?>" placeholder="<?php echo "Maximan Redeem Amount"; ?>" id="input-max_total" class="form-control" />
                </div>
              </div>
              <div class="form-group">
              ...
              <div class="form-group <?= $event_day_status ? '':'';?>">
                ...
                <div class="col-sm-10">
                  <input type="text" name="customer_group" value="" placeholder="<?php echo $entry_customer_group; ?>" id="input-customer_group" class="form-control" />
                  <div id="coupon-customer_group" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php foreach ($coupon_customer_group as $coupon_customer_group) { ?>
                    <div id="coupon-customer_group<?php echo $coupon_customer_group['customer_group_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $coupon_customer_group['name']; ?>
                      <input type="hidden" name="coupon_customer_group[]" value="<?php echo $coupon_customer_group['customer_group_id']; ?>" />
                    </div>
                    <?php } ?>
                  </div>
                </div>
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label" for="input-brand"><span data-toggle="tooltip" title="<?php echo $help_brand; ?>"><?php echo $entry_manufacturer; ?></span></label>
                <div class="col-sm-10">
                  <input type="text" name="brand" value="" placeholder="<?php echo $entry_manufacturer; ?>" id="input-brand" class="form-control" />
                  <div id="coupon-brand" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php foreach ($coupon_brand as $coupon_brand) { ?>
                    <div id="coupon-brand<?php echo $coupon_brand['manufacturer_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $coupon_brand['name']; ?>
                      <input type="hidden" name="coupon_brand[]" value="<?php echo $coupon_brand['manufacturer_id']; ?>" />
                    </div>
                    <?php } ?>
                  </div>
                </div>
...
$('input[name=\'brand\']').autocomplete({
	...
	'select': function(item) {
		...
		$('#coupon-brand' + item['value']).remove();
		
		$('#coupon-brand').append('<div id="coupon-brand' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="coupon_brand[]" value="' + item['value'] + '" /></div>');	
	}
});

$('#coupon-brand').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});

//Event Day Promotions Start
