...
class ModelReportSale extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, delivery_date, COUNT(*) AS `orders`, 
		...
		if (!empty($data['filter_fulfillment_date_start'])) {
			$sql .= " AND DATE(o.delivery_date) >= '" . $this->db->escape($data['filter_fulfillment_date_start']) . "'";
		}

		if (!empty($data['filter_fulfillment_date_end'])) {
			$sql .= " AND DATE(o.delivery_date) <= '" . $this->db->escape($data['filter_fulfillment_date_end']) . "'";
		}
		...
		switch($group) {
			case 'day';
				$sql .= " GROUP BY YEAR(o.date_added), MONTH(o.date_added), DAY(o.date_added), DAY(o.delivery_date)";
				break;
			default:
			case 'week':
				$sql .= " GROUP BY YEAR(o.date_added), WEEK(o.date_added), DAY(o.delivery_date)";
				break;
			case 'month':
				$sql .= " GROUP BY YEAR(o.date_added), MONTH(o.date_added), DAY(o.delivery_date)";
				break;
			case 'year':
				$sql .= " GROUP BY YEAR(o.date_added), DAY(o.delivery_date)";
				break;
		}
