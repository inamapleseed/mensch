...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
				if ($product_option['type'] == 'select' || $product_option['type'] == 'quantity' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
		...
		if (isset($data['product_special'])) {
			foreach ($data['product_special'] as $product_special) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_special SET product_id = '" . (int)$product_id . "', customer_group_id = '" . (int)$product_special['customer_group_id'] . "', priority = '" . (int)$product_special['priority'] . "', price = '" . (float)$product_special['price'] . "', date_start = '" . $this->db->escape($product_special['date_start']) . "', time_start = '" . date("H:i:s", strtotime($product_special['time_start'])) . "', date_end = '" . $this->db->escape($product_special['date_end']) . "', time_end = '" . date("H:i:s", strtotime($product_special['time_end'])) . "'");
			}
		}
		...
		if (isset($data['product_spec'])) {
			foreach ($data['product_spec'] as $product_spec) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_specs SET product_id = '" . (int)$product_id . "', include = '" . $this->db->escape($product_spec['include']) . "', dimage = '" . $this->db->escape($product_spec['dimage']) . "', title = '" . $this->db->escape($product_spec['title']) . "'");
			}
		}
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET redeem_point = '" . $this->db->escape($data['redeem_point']) . "', model = '" . $this->db->escape($data['model']) . "', info1 = '" . $this->db->escape($data['info1']) . "', include_info1 = '" . $this->db->escape($data['include_info1']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_specs WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProductSpecs($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_specs WHERE product_id = '" . (int)$product_id . "'");
