...
class ModelCatalogOption extends Model {
	public function addOption($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', sort_order = '" . (int)$data['sort_order'] . "', inclusive_discount = '".(int)$data['inclusive_discount']."'");
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', o_price = '".$option_value['o_price']."', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
	...
	public function editOption($option_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', sort_order = '" . (int)$data['sort_order'] . "', inclusive_discount = '".(int)$data['inclusive_discount']."' WHERE option_id = '" . (int)$option_id . "'");
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				if ($option_value['option_value_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_value_id = '" . (int)$option_value['option_value_id'] . "', option_id = '" . (int)$option_id . "', o_price = '".$option_value['o_price']."', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', o_price = '".$option_value['o_price']."', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
				}
				...
				if($option_value['o_price'] > 0){
			        $this->db->query("UPDATE ".DB_PREFIX."product_option_value SET price = '".$option_value['o_price']."' WHERE option_value_id = '" . (int)$option_value['option_value_id'] . "'");
				}
	...
	public function getOptions($data = array()) {
		...
		if (!empty($data['filter_name'])) {
			...
			$sql .= " AND LCASE(od.name) LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}
