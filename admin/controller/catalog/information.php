...
class ControllerCatalogInformation extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['informations'][] = array(
				...
				'href' 		  => str_replace(HTTPS_SERVER, HTTPS_CATALOG, $this->url->link('information/information&information_id=' . $result['information_id'])),
	...
	protected function getForm() {
		...
		$data['entry_repeater_row'] = $this->language->get('entry_repeater_row');
		$data['entry_row_image'] = $this->language->get('entry_row_image');
		$data['entry_row_type'] = $this->language->get('entry_row_type');
		...
		$data['row_types'] = array(
			array(
				'value' => 'image_left',
				'name' => $this->language->get('row_type_1'),
			),
			array(
				'value' => 'image_right',
				'name' => $this->language->get('row_type_2'),
			),
			array(
				'value' => 'fullwidth_top',
				'name' => $this->language->get('row_type_3'),
			),
			array(
				'value' => 'fullwidth_bottom',
				'name' => $this->language->get('row_type_4'),
			),
		);
		...
		$data['with_row_type'] = array(8);

		if (isset($this->request->post['information_repeaters'])) {
			$information_repeaters = $this->request->post['information_repeaters'];
		} elseif (isset($this->request->get['information_id'])) {
			$information_repeaters = $this->model_catalog_information->getInformationRepeater($this->request->get['information_id']);
		} else {
			$information_repeaters = array();
		}
		$this->load->model('tool/image');

		$data['information_repeaters'] = array();
		if(isset($information_repeaters) && $information_repeaters) {
		
			foreach ($information_repeaters as $language_id => $information_repeater) {

				foreach ($information_repeater as $repeater) {

					if (is_file(DIR_IMAGE .  $repeater['image'])) {
						$image = $repeater['image'];
						$thumb = $repeater['image'];
					} else {
						$image = '';
						$thumb = 'no_image.png';
					}
		
					$data['information_repeaters'][$language_id][] = array(
						'information_repeater_id'  =>   $repeater['information_repeater_id'],
						'row_type' =>  $repeater['row_type'],
						'image'      => $image,
						'thumb'      => $this->model_tool_image->resize($thumb, 100, 100),
						'description' =>  $repeater['description'],
						'is_brand' =>  $repeater['is_brand'],
					);
				}
			}
		}
		...
		$data['template_types'] = $this->model_catalog_information->getTemplateType();

		if (isset($this->request->post['template_type'])) {
			$data['template_type'] = $this->request->post['template_type'];
		} elseif (isset($this->request->get['information_id'])) {
			$data['template_type'] =  $information_info['template_type'];
		} else {
			$data['template_type'] = '';
		}
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['information_description'] as $language_id => $value) {
			...
			elseif(trim($value['meta_title']) == "" || (utf8_strlen($value['meta_title']) < 3)){
				$value['meta_title'] = $this->request->post['information_description'][$language_id]["meta_title"] = $value['title'];
			}
		...
		if(utf8_strlen($this->request->post['keyword']) < 1 && !isset($this->error["title"][1])){
			$this->request->post['keyword'] = generateSlug($this->request->post['information_description'][1]["title"]);
		}
