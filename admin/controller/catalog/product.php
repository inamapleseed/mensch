...
class ControllerCatalogProduct extends Controller {
		...
		foreach ($results as $result) {
			...
			foreach ($product_specials  as $product_special) {
				if (($product_special['date_start'] == '0000-00-00' || strtotime($product_special['date_start']) <= strtotime(date("Y-m-d"))) && ($product_special['date_end'] == '0000-00-00' || strtotime($product_special['date_end']) >= strtotime(date("Y-m-d")))) {
                    ...
                    if($product_special['date_start'] == '0000-00-00' && $product_special['date_end'] == '0000-00-00'){
					    $special = $product_special['price'];
					    break;
                    }
				}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['redeem_point'])) {
			$data['redeem_point'] = $this->request->post['redeem_point'];
		} elseif (!empty($product_info)) {
			$data['redeem_point'] = $product_info['redeem_point'];
		} else {
			$data['redeem_point'] = '0';
		}
		...
		foreach ($product_specials as $product_special) {
			$data['product_specials'][] = array(
				...
				'time_start'        => ($product_special['time_start'] != '') ? $product_special['time_start'] : '',
		...
		// Specs
		if (isset($this->request->post['product_spec'])) {
			$product_specs = $this->request->post['product_spec'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_specs = $this->model_catalog_product->getProductSpecs($this->request->get['product_id']);
		} else {
			$product_specs = array();
		}

		$data['product_specs'] = array();

		foreach ($product_specs as $product_spec) {
			$data['product_specs'][] = array(
				'title' => $product_spec['title'],
				'dimage' => $product_spec['dimage'],
				'include' => $product_spec['include'],
			);
		}
		...
		$data['xbundle_tab'] = $this->load->controller('extension/module/xbundle/getBundleTab');
