...
class ControllerExtensionModuleCalendar extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('calendar', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/calendar', 'token=' . $this->session->data['token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/calendar', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$data['calendar_events'] = $this->model_catalog_calendar_event->getCalendarEvents();

		if (isset($this->request->post['calendars'])) {
			$calendars = $this->request->post['calendars'];
		} elseif (isset($module_info['calendars'])) {
			$calendars = $module_info['calendars'];
		} else {
			$calendars = array();
		}
		...
		foreach ($calendars as $key => $value) {
			$data['calendars'][$key] = array(
				'title'         => $value['title'],
				'start_date'    => $value['start_date'],
				'end_date'      => $value['end_date'],
				'cevent_id'     => $value['cevent_id'],
			);
		}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/calendar')) {
