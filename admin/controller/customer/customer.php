...
// 			$mail->send();
			...
			if($this->config->get('pro_email_template_status')){
				...
				$email_params = array(
					'type' => 'customer.information.contact',
					'mail' => $mail,
					'reply_to' => $this->request->post['email'],
					'data' => array(
						'enquiry_telephone' => html_entity_decode($this->request->post['telephone'], ENT_QUOTES, 'UTF-8'),
						'enquiry_name' => html_entity_decode($this->request->post['firstname'] . ' ' . $this->request->post['lastname'], ENT_QUOTES, 'UTF-8'),
						'enquiry_mail' => html_entity_decode($this->request->post['email'], ENT_QUOTES, 'UTF-8'),
						'enquiry_pw' => html_entity_decode($this->request->post['password'], ENT_QUOTES, 'UTF-8'),
					),
				);
	...
	protected function getList() {
		...
		$data['sort_reward'] = $this->url->link('customer/customer', 'token=' . $this->session->data['token'] . '&sort=c.reward' . $url, true);
	...
	protected function getForm() {
		...
		$data['tab_customer_info'] = $this->language->get('tab_customer_info');
			...
			foreach ($this->request->post['address'] as $key => $value) {
				...
				*/
				...
				if (!isset($value['zone_id']) || $value['zone_id'] == '') {
					$this->error['address'][$key]['zone'] = sprintf($this->language->get('error_zone'), $this->language->get('entry_zone'), '');
				}
