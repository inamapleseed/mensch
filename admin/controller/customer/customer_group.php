...
class ControllerCustomerCustomerGroup extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['min_spend_amount'])) {
			$data['min_spend_amount'] = $this->request->post['min_spend_amount'];
		} elseif (!empty($customer_group_info)) {
			$data['min_spend_amount'] = $customer_group_info['min_spend_amount'];
		} else {
			$data['min_spend_amount'] = 0;
		}
		...
		//Loyalty Program Crons
		$data['loyalty_crons'] = "";
		$loyalty_status = $this->config->get('loyalty_program_status');
		if(isset($loyalty_status) && $loyalty_status == 1){
			$data['loyalty_crons'] = '<br><br>Loyalty Program Cron Job<br>wget -qO- '.HTTP_CATALOG . 'index.php?route=_cron/cron/checkMembership &> /dev/null';
			$data['loyalty_crons'] .= '<br>wget -qO- '.HTTP_CATALOG . 'index.php?route=_cron/cron/notifyMembership &> /dev/null';
		}

		//Event Day Promotions  Crons
		$data['event_day_crons'] = "";
		$event_day_status = $this->config->get('setting_birthdaycoupon_status');
		if(isset($event_day_status) && $event_day_status == 1){
			$data['event_day_crons'] = '<br><br>Event Day Promotions Cron Job<br>wget -qO- '.HTTP_CATALOG . 'index.php?route=_cron/cron/generateBirthdayCoupon &> /dev/null';
		}
