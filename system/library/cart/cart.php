...
class Cart {
	...
	public function getProducts($cart_id = 0) {
        ...
        if($cart_id == 0){
    		$cart_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' order by cart_id ASC");
        }else{
            $cart_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND cart_id = '" . $cart_id . "' order by cart_id ASC");
        }
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
								...
								foreach ($value as $product_option_value_id => $quantity) {			
									...
									if ($option_value_query->num_rows) {
										...
										$option_data[] = array(
											...
											'option_id'               => $option_query->row['option_id'],
											'option_value_id'         => $option_value_query->row['option_value_id'],
											'name'                    => $option_value_query->row['name'],
											'value'                   => $quantity[0],
											'type'                    => $option_query->row['type'],
											'quantity'                => $option_value_query->row['quantity'],
											'subtract'                => $option_value_query->row['subtract'],
											'price'                   => ($option_value_query->row['price'] * $quantity[0]),
											'price_prefix'            => $option_value_query->row['price_prefix'],
											'points'                  => $option_value_query->row['points'],
											'points_prefix'           => $option_value_query->row['points_prefix'],
											'weight'                  => $option_value_query->row['weight'],
				...
				if (property_exists($this, 'ocmprice')) {
                    $_ocmprice_data = array(
                        ...
                        'carts'        => VERSION >= '2.1.0.0' ? $cart_query->rows : $this->session->data['cart'],
                        'cart'         => VERSION >= '2.1.0.0' ? $cart : $product,
                    ...
                    if (VERSION >= '2.1.0.0' && (int)$product_query->row['quantity'] < $cart['quantity']) {
                        $stock = false;
                    }
                }
                // X-bundle
				...
				$product_data[] = array(
					...
					'redeem_point'    => $product_query->row['redeem_point'],
					'model'           => $product_query->row['model'],
					'sku'			  => $sku,
					'shipping'        => $product_query->row['shipping'],
					'image'           => $option_image ?  $option_image : $product_query->row['image'],
					'option'          => $option_data,
	...
	public function checkOptionStock($product_id, $quantity = 1, $option = array(), $recurring_id = 0) {
		/* completecombo */
		...
		$query = $this->db->query("SELECT COUNT(*), quantity AS total FROM " . DB_PREFIX . "cart WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0) {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, cart_id FROM " . DB_PREFIX . "cart WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "' AND redeem = '0'");
		...
		if (!$query->row['total']) {
			...
			$this->session->data['new_to_cart'] = $query->row['cart_id'];
		}
		/* completecombo */
	...
	public function updateCart($product_id, $quantity = 1, $option = array(), $recurring_id = 0, $cart_id) {
	  ...
	  $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "cart WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND cart_id = '" . $cart_id . "'");

	  if (!$query->row['total']) {
		  $this->db->query("INSERT " . DB_PREFIX . "cart SET api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "', customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', date_added = NOW()");
	  } else {
		  $this->db->query("UPDATE " . DB_PREFIX . "cart SET api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "', customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "' WHERE cart_id = '" . $cart_id . "'");
	  }
	  /* completecombo */
		if($this->config->get("offerpage_installed")) {
			$this->sortcombination();
	...
	public function update($cart_id, $quantity) {
		...
		// X-bundle
		if (property_exists($this, 'ocmprice')) {
			$_ocmprice = $this->ocmprice->onCartOperation((VERSION >= '2.1.0.0' ? $cart_id : $key), __FUNCTION__);
	...
	public function getProductOptionImageByProductID($product_id,$product_option_value_id) {
		$query = $this->db->query("SELECT * FROM (SELECT DISTINCT(product_option_value_id), product_id, image FROM " . DB_PREFIX . "product_option_image WHERE product_id = '" . (int)$product_id . "' AND sort_order <= 1 ORDER BY sort_order ASC) as oimg  WHERE product_option_value_id ='".(int)$product_option_value_id."' GROUP BY (oimg.product_option_value_id) ");
 ...
 //      	$cartquery = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE cart_id = '".(int)$cart_id."'");
	...
	public function redeem($cart_id, $redeem_all) {
		...
		if($redeem_all == 1){
			...
			if($query2->num_rows > 0){
				$this->db->query("UPDATE " . DB_PREFIX . "cart SET quantity = (quantity + " . $query->row['quantity'] . "), reward_point = '" . $query_product->row['redeem_point'] . "' WHERE cart_id = '" . $query2->row['cart_id'] . "'");
				$this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE cart_id = '" . $cart_id . "'");
			}else{
			...
			$query2 = $this->db->query($sql2);
			if($query2->num_rows > 0){
				$this->db->query("UPDATE " . DB_PREFIX . "cart SET quantity = (quantity + 1), reward_point = '" . $query_product->row['redeem_point'] . "' WHERE cart_id = '" . $query2->row['cart_id'] . "'");
				if($query->row['quantity'] > 1){
					$this->db->query("UPDATE " . DB_PREFIX . "cart SET quantity = (quantity - 1) WHERE cart_id = '" . $cart_id . "'");
				}else{
					$this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE cart_id = '" . $cart_id . "'");
				}
			}else{
				$this->db->query("UPDATE " . DB_PREFIX . "cart SET quantity = (quantity - 1) WHERE cart_id = '" . $cart_id . "'");
				$this->db->query("INSERT " . DB_PREFIX . "cart SET api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "', "
	...
	public function getUsedPoint(){
		...
		foreach ($cart_query->rows as $cart) {
			if($cart['redeem'] == 1){
				$point = $point + ($cart['reward_point'] * $cart['quantity']);
			}
		}
		return $point;
	}
