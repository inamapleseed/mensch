...
final class XBundle {
    ...
    public function getBundlePrice($cart, $product, $special, $option_data) {
        ...
        $bundle_info = $this->db->query("SELECT * FROM " . DB_PREFIX . "xbundle WHERE product_id = '".(int)$option['xbundle']."' AND bundle_id = '".(int)$bundle_id."'")->row;
        // debug($bundle_info);
        ...
        if ($bundle_info) {
            ...
            $discount = trim(trim($bundle_info['discount']), '-');
            if (substr($bundle_info['discount'], -1) == '%') {
                $bundle_info['discount'] = rtrim($bundle_info['discount'],'%');
                $percent = true;
                $value = (float)$bundle_info['discount'] / 100;
            } else {
                $percent = false;
                $value = (float)$bundle_info['discount'];
            }
            $amount = $percent ? ($value * $original_price) : $value;
